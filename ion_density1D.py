...
print(f"陽離子數量 (BMI): {len(cation)}")
print(f"陰離子數量 (TRF): {len(anion)}")
...
plt.plot(z_positions, cation_Density, label="Cation (BMI)", color='blue', linewidth=1.5)
...
# 單獨繪製陰離子密度圖
plt.figure(figsize=(12, 6), dpi=300)
plt.plot(z_positions, anion_Density, label="Anion (TRF)", color='red', linewidth=1.5)
