...
def plot_density_single(z_pos, density, label, color, title, filename, config):
...
def plot_density_combined(z_pos, cat_dens, an_dens, cat_name, an_name, config):
    ...
    fig.tight_layout()
    save_plot(fig, "Ion_Density_Combined_V3.png", config)
    ...
    ax.set_title("Ion Density Comparison", fontsize=14, fontweight='bold')
    ax.set_xlabel("Z Position (Å)", fontsize=12)
    ax.set_ylabel("Number Density (Å⁻¹)", fontsize=12)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3)
    ...
    fig2.tight_layout()
    save_plot(fig2, "Ion_Density_Comparison_V3.png", config)
...
def main():
    ...
    plot_density_single(
        ...
        "Anion_Density_V3.png", config
    )
    
    plot_density_combined(z_pos, cat_dens, an_dens, cat_name, an_name, config)
